...
<div class="cards">
<?php if (canAccessAdminCategory('estimates')): ?><a class="metric primary" href="./estimates.php"><div class="label">누적 견적</div><strong><?=number_format($estimateTotal)?></strong><small>차량견적 <?=number_format($directTotal)?> · 간편견적 <?=number_format($quickTotal)?></small></a><?php endif; ?>
<?php if (canAccessAdminCategory('estimates')): ?><a class="metric" href="./estimates.php"><div class="label">이번 달 견적</div><strong><?=number_format($estimateMonth)?></strong><small>오늘 <?=number_format($estimateToday)?>건</small></a><?php endif; ?>
<?php if (canAccessAdminCategory('inquiries')): ?><a class="metric" href="./inquiries.php"><div class="label">누적 고객문의</div><strong><?=number_format($inquiryTotal)?></strong><small>이번 달 <?=number_format($inquiryMonth)?>건</small></a><?php endif; ?>
<?php if (canAccessAdminCategory('estimates')): ?><a class="metric warn" href="./estimates.php?status=NEW"><div class="label">상담 대기</div><strong><?=number_format($estimateStatusCounts['NEW'])?></strong><small>신규 견적</small></a><?php endif; ?>
<?php if (canAccessAdminCategory('estimates')): ?><a class="metric green" href="./estimates.php?status=CONTACTED"><div class="label">진행중 견적</div><strong><?=number_format($activeEstimate)?></strong><small>신규~승인 단계</small></a><?php endif; ?>
<?php if (canAccessAdminCategory('estimates')): ?><a class="metric" href="./estimates.php?status=CONTRACTED"><div class="label">계약 완료</div><strong><?=number_format($estimateStatusCounts['CONTRACTED'])?></strong><small>누적 계약률 <?=$contractRate?>%</small></a><?php endif; ?>
</div>

<?php if (canAccessAdminCategory('estimates')): ?><section class="panel"><div class="panel-head"><h2>견적 진행 현황</h2><a href="./estimates.php">견적문의 전체보기</a></div><div class="funnel"><?php foreach($statusOrder as $k): ?><a class="step" href="./estimates.php?status=<?=$k?>" style="text-decoration:none;color:inherit"><span><?=$statusLabels[$k]?></span><b><?=number_format($estimateStatusCounts[$k])?></b><small>건</small></a><?php endforeach;?></div><?php if($estimateCanceled>0): ?><div style="margin-top:10px;color:#8b99a2;font-size:14px">취소 <?=number_format($estimateCanceled)?>건</div><?php endif;?></section><?php endif; ?>

<div class="section-grid">
<section class="panel"><div class="panel-head"><h2>최근 7일 유입 추이</h2><span style="font-size:14px;color:#8999a2">견적문의 / 고객문의</span></div><?php $maxTrend=max(array_map(fn($r)=>max($r['estimate'],$r['inquiry']),$trend)?:[1]);$maxTrend=max(1,$maxTrend);?><div class="trend"><?php foreach($trend as $r):?><div class="trend-day"><div class="trend-bars"><i class="tb direct" style="height:<?=max(3,round($r['estimate']/$maxTrend*135))?>px"></i><i class="tb quick" style="height:<?=max(3,round($r['inquiry']/$maxTrend*135))?>px"></i></div><b style="font-size:14px"><?=number_format($r['estimate']+$r['inquiry'])?></b><small><?=date('m/d',strtotime($r['date']))?></small></div><?php endforeach;?></div><div class="legend"><span><i style="background:#4b35c5"></i>견적문의</span><span><i style="background:#47a0d8"></i>고객문의</span></div></section>
...
<div class="two-list">
<?php if (canAccessAdminCategory('estimates')): ?><section class="panel"><div class="panel-head"><h2>최근 견적문의</h2><a href="./estimates.php">전체보기</a></div><div class="table-wrap"><table class="table"><thead><tr><th>구분</th><th>고객</th><th>차량/관심차종</th><th>상태</th><th>신청일</th></tr></thead><tbody><?php foreach($recentEstimates as $r):?><tr><td><span class="pill"><?=($r['src']==='QUICK'?'간편견적':'차량견적')?></span></td><td><a href="./estimate-detail.php?type=<?=strtolower($r['src'])?>&id=<?=(int)$r['id']?>"><b><?=ag_h($r['customer_name'])?></b></a></td><td class="ellipsis"><?=ag_h(trim((string)$r['item'])?:'-')?></td><td><span class="pill"><?=ag_h($statusLabels[$r['status']]??$r['status'])?></span></td><td><?=date('m/d H:i',strtotime($r['created_at']))?></td></tr><?php endforeach;?><?php if(!$recentEstimates):?><tr><td colspan="5" class="empty">견적문의가 없습니다.</td></tr><?php endif;?></tbody></table></div></section><?php endif; ?>
<?php if (canAccessAdminCategory('inquiries')): ?><section class="panel"><div class="panel-head"><h2>최근 고객문의</h2><a href="./inquiries.php">전체보기</a></div><div class="table-wrap"><table class="table"><thead><tr><th>고객</th><th>문의내용</th><th>상태</th><th>등록일</th></tr></thead><tbody><?php foreach($recentInquiries as $r):?><tr><td><b><?=ag_h($r['member_name']?:'비회원')?></b></td><td class="ellipsis"><a href="./inquiries.php?open=<?=(int)$r['id']?>"><?=ag_h($r['subject']?:'-')?></a></td><td><span class="pill"><?=($r['status']==='ANSWERED'?'답변완료':'미처리')?></span></td><td><?=date('m/d H:i',strtotime($r['created_at']))?></td></tr><?php endforeach;?><?php if(!$recentInquiries):?><tr><td colspan="4" class="empty">고객문의가 없습니다.</td></tr><?php endif;?></tbody></table></div></section><?php endif; ?>
</div>

<?php if(canAccessAdminCategory('estimates') && $popularVehicles):?><section class="panel"><div class="panel-head"><h2>차량견적 인기 차량 TOP 6</h2><a href="./estimates.php?type=DIRECT">차량견적 보기</a></div><?php $pvmax=max(1,max(array_map(fn($r)=>(int)$r['cnt'],$popularVehicles))); foreach($popularVehicles as $r):?><div class="status-row"><span><?=ag_h($r['vehicle'])?></span><div class="bar"><i style="width:<?=((int)$r['cnt']/$pvmax*100)?>%"></i></div><b><?=number_format((int)$r['cnt'])?></b></div><?php endforeach;?></section><?php endif;?>

<section class="panel"><div class="panel-head"><h2>빠른 관리</h2></div><div class="quick-links"><?php if (canAccessAdminCategory('estimates')): ?><a href="./estimates.php">견적문의 관리</a><?php endif; ?><?php if (canAccessAdminCategory('inquiries')): ?><a href="./inquiries.php">고객문의 관리</a><?php endif; ?><?php if (canAccessAdminCategory('customers')): ?><a href="./customers.php">고객 관리</a><?php endif; ?></div></section>
</main></div></body></html>
