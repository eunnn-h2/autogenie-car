...
.admin-user-area .logout-link{margin-top:4px;font-size:10px;color:#25bcd0;text-decoration:none}
.admin-user-area .logout-link:hover{text-decoration:underline}

.role-badge{display:inline-block;margin-top:4px;padding:2px 5px;border-radius:3px;background:#eef2ff;color:#4338ca;font-size:9px;font-weight:800}
...
@media(max-width:900px){
    ...
    .logo-area{height:56px;margin-bottom:8px}
    .menu-section{margin-bottom:8px}
    .menu-section>p{margin-top:9px}
    .menu-section>a,.menu-main{padding:8px 10px}
    ...
    <aside class="sidebar">
        <div class="logo-area">
            ...
            <div class="admin-user-area">
                ...
                <small class="role-badge"><?= htmlspecialchars(adminRole(), ENT_QUOTES, 'UTF-8') ?></small>
                <a class="logout-link" href="./logout.php">로그아웃</a>
            </div>
        </div>

        <div class="menu-section">
            <p>상품</p>
            <a class="menu-main active" href="./index.php#product-list">▣ 상품</a>
            <div class="submenu">
                <a class="active" href="./index.php#product-list">전체조회</a>
                <a href="#bulk-import">엑셀 일괄등록</a>
            </div>
        </div>

        <div class="menu-section">
            <p>견적</p>
            <a href="./estimates.php">견적 신청 관리</a>
            <a href="../db-test.html" target="_blank">사용자 견적 화면</a>
        </div>

        <?php if (isSuperAdmin()): ?>
        <div class="menu-section">
            <p>관리자</p>
            <a href="./admins.php">관리자 계정 관리</a>
        </div>
        <?php endif; ?>

        <div class="menu-section">
            <p>데이터</p>
            <a href="../db-test.html" target="_blank">DB 출력 테스트</a>
            <a href="../api/test-car-data.php" target="_blank">API 확인</a>
            <a href="http://localhost/phpmyadmin/" target="_blank">phpMyAdmin</a>
        </div>

        <div class="sidebar-stats">
            <div><span>브랜드</span><b><?= number_format($dashboardCounts['brands'] ?? 0) ?></b></div>
            <div><span>차량</span><b><?= number_format($dashboardCounts['vehicles'] ?? 0) ?></b></div>
            <div><span>색상</span><b><?= number_format($dashboardCounts['colors'] ?? 0) ?></b></div>
            <div><span>트림</span><b><?= number_format($dashboardCounts['trims'] ?? 0) ?></b></div>
        </div>
    </aside>
