...
if ($type === '' || $type === 'DIRECT') {
    ...
    $sql = 'SELECT e.* FROM estimates e' . ($where ? ' WHERE ' . implode(' AND ', $where) : '') . ' ORDER BY e.id DESC LIMIT 500';
    try {
...
if ($type === '' || $type === 'QUICK') {
    ...
    $sql = 'SELECT q.* FROM quick_estimates q' . ($where ? ' WHERE ' . implode(' AND ', $where) : '') . ' ORDER BY q.id DESC LIMIT 500';
    try {
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        foreach ($stmt->fetchAll() as $r) {
